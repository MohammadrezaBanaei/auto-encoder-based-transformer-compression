...
class ModelArguments:
    ...
    pretrained_ae_dir: str = field(
        default=None, metadata={"help": "Path to pretrained AutoEncoder models"}
    )
    svd: bool = field(
...
def main():
    ...
    if model_args.pretrained_ae_dir is not None:
        ...
        with open(os.path.join(model_args.pretrained_ae_dir, 'config.yml'), 'r') as stream:
            ae_config = yaml.load(stream, Loader=yaml.FullLoader)
        with open(os.path.join(model_args.pretrained_ae_dir, 'checkpoints', 'transformer_bias_data.pkl'), 'rb') as f:
            transformer_bias_data = pickle.load(f)

        # SUBSTITUTE WORD EMBEDDINGS
        if ae_config["dataset"]["to_compress"]['word_embeddings']:
            assert os.path.exists(os.path.join(model_args.pretrained_ae_dir,
                                               'checkpoints',
            ...
            emb_ae_model.load_state_dict(torch.load(os.path.join(model_args.pretrained_ae_dir, 'checkpoints',
                                                                 'word_embeddings', 'model.pt')))
            emb_weights = torch.load(os.path.join(model_args.pretrained_ae_dir, 'checkpoints',
            ...
            if ae_config["dataset"]["to_compress"][temp_name_mapping[proj_name]]:
                if ae_config['modules_training_setting'] == 'separated':
                    for layer_idx in range(model.bert.config.num_hidden_layers):
                        ...
                        ae_path = os.path.join(model_args.pretrained_ae_dir,
                                               'checkpoints',
                        ...
                        if proj_name == 'attention_output':
                            setattr(model.bert.encoder.layer[layer_idx].attention.output, 'dense', sub_module)
                        elif proj_name == 'output_dense':
                    ...
                    ae_path = os.path.join(model_args.pretrained_ae_dir, 'checkpoints', module_name)
        ...
        with open(os.path.join(model_args.svd_config_dir, 'config.yml'), 'r') as stream:
            svd_config = yaml.load(stream, Loader=yaml.FullLoader)

        svd_iters_dict = svd_config['training']['svd_best_it']
        svd_cr_dict = svd_config['training']['svd_cr']

        logger.info('Using SVD compression...')

        coefficients_name = get_coefficients_name(svd_config)

        if model_args.svd_compression_mode == 'separated':
            for module_name, best_it in svd_iters_dict.items():
                svd_cr = svd_cr_dict[module_name]
                if module_name == 'word_embeddings' and svd_config['dataset']['to_compress']['word_embeddings'] == True:
                    coefs = get_coefficients(coefficients_name, 'word_embeddings', svd_config, compute=False)
                    input_matrix = model.bert.embeddings.word_embeddings.weight.detach()
                    svd_word_emb_module = MFWordEmbeddingModule.get_svd_emb_module(input_matrix=input_matrix.detach(),
                                                                                   compression_ratio=svd_cr,
                                                                                   n_iter=int(best_it),
                ...
                elif 'layer' in module_name:
                    m = re.match(r"layer_([0-9]+)_(\w+)$", module_name)
                    layer_idx = int(m.group(1))
                    proj_name = m.group(2)
                    coefs = get_coefficients(coefficients_name, 'layer', svd_config, submodule_name=proj_name, compute=False)
                    if coefs is not None:
                        coefs = coefs[layer_idx]
                    if proj_name == 'key':
                        if svd_config['dataset']['to_compress']['all_keys'] == False:
                            continue
                        input_matrix = model.bert.encoder.layer[layer_idx].attention.self.key.weight
                        module_bias = model.bert.encoder.layer[layer_idx].attention.self.key.bias
                    elif proj_name == 'query':
                        if svd_config['dataset']['to_compress']['all_queries'] == False:
                            continue
                        input_matrix = model.bert.encoder.layer[layer_idx].attention.self.query.weight
                        module_bias = model.bert.encoder.layer[layer_idx].attention.self.query.bias
                    elif proj_name == 'value':
                        if svd_config['dataset']['to_compress']['all_values'] == False:
                            continue
                        input_matrix = model.bert.encoder.layer[layer_idx].attention.self.value.weight
                        module_bias = model.bert.encoder.layer[layer_idx].attention.self.value.bias
                    elif proj_name == 'attention_output':
                        if svd_config['dataset']['to_compress']['all_attention_outputs'] == False:
                            continue
                        input_matrix = model.bert.encoder.layer[layer_idx].attention.output.dense.weight
                        module_bias = model.bert.encoder.layer[layer_idx].attention.output.dense.bias
                    elif proj_name == 'output_dense':
                        if svd_config['dataset']['to_compress']['all_output_denses'] == False:
                            continue
                        input_matrix = model.bert.encoder.layer[layer_idx].output.dense.weight
                        module_bias = model.bert.encoder.layer[layer_idx].output.dense.bias
                    elif proj_name == 'intermediate_dense':
                        if svd_config['dataset']['to_compress']['all_intermediate_denses'] == False:
                            continue
                        input_matrix = model.bert.encoder.layer[layer_idx].intermediate.dense.weight
                        module_bias = model.bert.encoder.layer[layer_idx].intermediate.dense.bias

                    sub_module = MFWeightModule.get_svd_mf_module(input_matrix=input_matrix.detach(),
            ...
            raise NotImplementedError('Only separated mode implemented for now for SVD')

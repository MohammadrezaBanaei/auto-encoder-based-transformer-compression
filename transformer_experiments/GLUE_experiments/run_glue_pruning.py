...
from main import get_pruned_module
...
class ModelArguments:
    ...
    compression_ratio: float = field(
        default=None,
        metadata={"help": "Compression ratio for module pruning."},
    )
    pruning_module: str = field(
        default="word_embeddings",
        metadata={"help": "The name of the module to be compressed."},
    )
...
def main():
    ...
    if True:
        logger.info('Using pruning compression...')

        # SUBSTITUTE WORD EMBEDDINGS
        if model_args.pruning_module == "word_embeddings":
            original_module = model.bert.embeddings.word_embeddings
            reconstructed_modules_dict['word_embeddings'] = {}
            reconstructed_modules_dict['word_embeddings']['original_module'] = original_module.weight
            pruned_module = prune.l1_unstructured(original_module, 'weight', 1 - 1/model_args.compression_ratio)
            reconstructed_modules_dict['word_embeddings']['reconstructed_module'] = pruned_module.weight
            # model.bert.embeddings.word_embeddings.weight = torch.nn.Parameter(temp_pruned_embedding)

        # SUBSTITUTE MATRICES
        for proj_name in ["key", "value", "query", "attention_output", "output_dense", "intermediate_dense"]:
            if model_args.pruning_module == proj_name:
                for layer_idx in range(model.bert.config.num_hidden_layers):
                    ...
                    if proj_name == 'key':
                        original_module = model.bert.encoder.layer[layer_idx].attention.self.key
                        reconstructed_modules_dict[module_name]['original_module'] = original_module.weight
                    elif proj_name == 'query':
                        original_module = model.bert.encoder.layer[layer_idx].attention.self.query
                        reconstructed_modules_dict[module_name]['original_module'] = original_module.weight
                    elif proj_name == 'value':
                        original_module = model.bert.encoder.layer[layer_idx].attention.self.value
                        reconstructed_modules_dict[module_name]['original_module'] = original_module.weight
                    elif proj_name == 'attention_output':
                        original_module = model.bert.encoder.layer[layer_idx].attention.output.dense
                        reconstructed_modules_dict[module_name]['original_module'] = original_module.weight
                    elif proj_name == 'output_dense':
                        original_module = model.bert.encoder.layer[layer_idx].output.dense
                        reconstructed_modules_dict[module_name]['original_module'] = original_module.weight
                    elif proj_name == 'intermediate_dense':
                        original_module = model.bert.encoder.layer[layer_idx].intermediate.dense
                        reconstructed_modules_dict[module_name]['original_module'] = original_module.weight
                    else:
                        raise ValueError("Critical bug in pruning process!")

                    pruned_module = prune.l1_unstructured(original_module, 'weight', 1 - 1/model_args.compression_ratio)
                    reconstructed_modules_dict[module_name]['reconstructed_module'] = pruned_module.weight
